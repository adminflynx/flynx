		...
		<div class="fixed w-full z-30 flex bg-transparent p-2 items-center justify-center h-16 px-5">
			...
			<div
				...
				<button @click="openSide">
					<img class="block h-10 md:h-12 w-auto " src="../assets/logo.png" alt="ifc4All logo">
				</button>
				<div>
					<router-link class="ml-5" to="/"> FLynx </router-link>
				</div>
			</div>
				...
				<button @click="showUserMenu = !showUserMenu" class="flex space-x-3 items-center px-3 hover:opacity-80 transition-opacity">
					<div class="flex-none flex justify-center">
						<div class="w-8 h-8 flex">
							<mdicon :name="auth.isAuthenticated ? 'shield-check' : 'account-circle-outline'"
								:class="auth.isAuthenticated ? 'text-green-500' : 'text-gray-500 dark:text-gray-400'"
								class="shadow rounded-full object-cover" />
						</div>
					</div>
					<div class="hidden md:block text-sm text-black dark:text-white">
						{{ auth.isAuthenticated ? 'Admin' : 'Login' }}
					</div>
				</button>
				<!-- User dropdown -->
				<div v-if="showUserMenu"
					class="absolute top-14 right-0 w-48 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl z-50 overflow-hidden">
					<template v-if="auth.isAuthenticated">
						...
						<button @click="handleLogout"
							class="flex items-center gap-2 w-full px-4 py-3 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 border-t border-gray-200 dark:border-gray-700">
							<mdicon name="logout" size="18" /> Cerrar Sesión
						</button>
					</template>
					<template v-else>
						<router-link to="/admin/login" @click="showUserMenu = false"
							class="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700">
							<mdicon name="login" size="18" /> Admin Login
						</router-link>
					</template>
				</div>
			</div>
